...
Format legality will be stored in `card_identity_format_legalities` instead of a Commander-specific column on
`card_identities`. The format value remains source-defined text, while legality is constrained to Scryfall's known
legality states. The `oracle_cards` import replaces Card Identities and their format legalities in one transaction.

Card reference records will store source page URIs for user-facing links. Card Printings will store nullable
