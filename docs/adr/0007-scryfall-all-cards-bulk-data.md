...
The MVP will sync Scryfall card data from the All Cards and Oracle Cards bulk data exports. ManaBox collection exports include a Scryfall ID column, so importing All Cards allows CollectionCard rows to resolve directly to exact CardPrinting records. CardIdentity records should come from Scryfall's Oracle Cards bulk data because Scryfall already provides one representative card object per Oracle ID.
